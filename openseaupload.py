...
def main_program_loop():
    ...
    while end_num >= start_num:
        ...
        if is_explicit.get():
            expl = driver.find_element_by_xpath("//input[@id='explicit-content-toggle']")
            driver.execute_script("arguments[0].scrollIntoView();",expl)
            expl.click()
            time.sleep(0.5)    
...
isExplicit = tkinter.Checkbutton(root, text='Explicit Content', var=is_explicit)
isExplicit.grid(row=40, column=0)
